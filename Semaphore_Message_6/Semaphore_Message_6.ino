...
static SemaphoreHandle_t serial_mtx;  // Mutex for serial port critical section
static SemaphoreHandle_t count_sem;   // Semaphore tracking the # of empty spots in buffer
static SemaphoreHandle_t empty_sem;   // Semaphore tracking the # of elements in buffer
...
void setup() {
  ...
  count_sem = xSemaphoreCreateCounting(BUF_SIZE, 0);
